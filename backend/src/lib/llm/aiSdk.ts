 ...
 * it broke streaming.ts's `instanceof AssistantStreamAskInputsPause` check
 * (the ask_inputs pause is thrown inside runTools, i.e. inside the SDK's tool
 * `execute`) and its `instanceof UserFacingError` check.
 */
